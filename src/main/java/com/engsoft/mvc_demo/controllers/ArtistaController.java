...
public class ArtistaController {
    ...
    @GetMapping("/artistas")
    public String getArtista(@PathVariable String nome, Model model) {
        Optional<Artista> artista = artistas.stream()
            .filter(a -> a.getNome().equalsIgnoreCase(nome))
            .findFirst();

        if (artista.isPresent()) {
            model.addAttribute("artista", artista.get());
            model.addAttribute("musicas", artista.get().getMusicas());
            return "artistas"; 
        } else {
            return "erro"; // Template de erro simples (opcional)
        }
    }
}